    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          <NuxtLink to="/llm/completion" class="text-blue-500 hover:underline">
            ← Back to Completion
          </NuxtLink>
      ...
      <Card>
        ...
        <CardContent>
          <div class="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
            <div class="whitespace-pre-wrap text-sm">{{ requestData.input?.prompt || 'No prompt found' }}</div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card v-if="requestData.output">
        ...
        <CardContent>
          <div v-if="requestData.stream" class="space-y-4">
            ...
            <!-- Debug Information for Streaming Tokens -->
            <div v-if="requestData.output && requestData.stream" class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
              <h5 class="text-sm font-medium text-yellow-800 dark:text-yellow-200 mb-2">Streaming Token Debug Info</h5>
              <div class="text-xs space-y-1">
                <div><strong>Streaming tokens count:</strong> {{ streamingTokens.length }}</div>
                <div><strong>Total chunks:</strong> {{ requestData.output.total_chunks }}</div>
                <div><strong>Has logprobs in chunks:</strong> {{ hasLogprobsInChunks(requestData.output.chunks) ? 'Yes' : 'No' }}</div>
              </div>
            </div>
          </div>
          <div v-else class="space-y-4">
            ...
            <div v-if="requestData.output && !requestData.stream" class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
              <h5 class="text-sm font-medium text-yellow-800 dark:text-yellow-200 mb-2">Token Debug Info</h5>
              <div class="text-xs space-y-1">
                <div><strong>Non-streaming tokens count:</strong> {{ nonStreamingTokens.length }}</div>
                <div><strong>Has logprobs:</strong> {{ requestData.output.choices?.[0]?.logprobs ? 'Yes' : 'No' }}</div>
                <div v-if="requestData.output.choices?.[0]?.logprobs">
                  <strong>Logprobs structure:</strong>
                  <span v-if="requestData.output.choices[0].logprobs.tokens">Array format (tokens, token_logprobs, top_logprobs)</span>
                  <span v-else-if="requestData.output.choices[0].logprobs.content">Content format</span>
                  <span v-else>Other format</span>
                </div>
              </div>
            </div>
          </div>
...
import { parseLogprobsFromResponse, parseLogprobsFromStreamingChunks, extractTextFromTokens, hasLogprobsInChunks, type Token } from '@/composables/useLogprobs'
  ...
  try {
    ...
    if (response.output) {
      if (response.stream && response.output.chunks) {
        ...
        console.log('Parsed streaming tokens:', streamingTokens.value)
      } else if (response.output.choices) {
        // Parse non-streaming response
        nonStreamingTokens.value = parseLogprobsFromResponse(response.output)
        console.log('Parsed non-streaming tokens:', nonStreamingTokens.value)
        console.log('Response output structure:', response.output)
      }
    }
  } catch (err: any) {
...
const getNonStreamingResponseText = () => {
  if (!requestData.value?.output?.choices?.[0]?.text) return ''
  ...
  // Fallback to raw text extraction
  return requestData.value.output.choices[0].text
}
